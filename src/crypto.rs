...
pub fn encrypt(plaintext: &[u8], key: Key) -> Result<Vec<u8>> {
	...
	let ciphertext = key.cipher.encrypt(&nonce, plaintext).map_err(|e| anyhow!("encrypt: {e}"))?;
...
pub fn decrypt(ciphertext: &[u8], key: Key) -> Result<Vec<u8>> {
	...
	let nonce = Nonce::try_from(nonce_data)?;
	let plaintext = key.cipher.decrypt(&nonce, ciphertext).map_err(|e| anyhow!("decrypt: {e}"))?;
	Ok(plaintext)
}
